...
import { getAllowedSkillsForClass } from "@/engine/classes";
...
export function PlayerStateProvider({ children }) {
  ...
  // Get current class for action ownership validation
  const { classId } = useCurrentClass();
  const allowedSkillsRef = useRef(new Set());
  
  // Update allowed skills when class changes
  useEffect(() => {
    const allowed = getAllowedSkillsForClass(classId);
    allowedSkillsRef.current = new Set(allowed);
    
    if (import.meta.env.DEV) {
      console.log(`[PLAYER][${classId}] Allowed skills:`, allowed.length);
    }
  }, [classId]);
  ...
  const handleInput = useCallback((inputName, isPressed, isClick = false) => {
    ...
    if (isPressed) {
      // CLASS OWNERSHIP GUARD: Verify the action is owned by the active class
      const actionConfig = getActionById(inputName);
      if (actionConfig) {
        const skillId = actionConfig._skillId || actionConfig.id;
        if (!allowedSkillsRef.current.has(skillId)) {
          if (import.meta.env.DEV) {
            console.warn(`[GUARD] Attempted to use skill "${skillId}" not owned by active class`);
          }
          return; // Block execution - action not owned by current class
        }
      }
